import unittest
import numpy as np
from gage.nfg.battle_of_the_sexes import battle_of_the_sexes


class TestBattleOfTheSexes(unittest.TestCase):
    batch_size = 100
    min_r = 10
    max_r = 20
    seed = 0
    ...
    def test_battle_of_the_sexes(self):
        payoff_matrices = battle_of_the_sexes(batch_size=self.batch_size,
                                              min_r=self.min_r,
                                              max_r=self.max_r,
                                              seed=self.seed)
